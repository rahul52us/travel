...
const FaqHome = observer(() => {
  ...
  const handleSubmit = async (formData: any) => {
    setIsLoading(true);
    try {
      await updateCompanyDetails({ homeFaq: formData?.homeFaq });
      getCompanyDetails();
      toast({
        title: "FAQ Updated",
  ...
  return (
    <Box p={4}>
      ...
      {/* Existing Home FAQ Data - Accordion */}
      {companyDetails?.homeFaq?.length > 0 ? (
        <Box mb={6}>
          <Accordion allowMultiple>
            {companyDetails?.homeFaq.map((faq, index) => (
